...
sealed trait InputValidators {
  ...
  def validateEmail(email: String): ValidationResult[String] =
    if (email.matches("""^[a-zA-Z0-9\.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$""")) email.validNec else EmailHasSpecialCharacters.invalidNec
